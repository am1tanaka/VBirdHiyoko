...
ある機能を利用するためにクラスのインスタンスを取得することは、UnityではGetComponentを使ってよく行います。このときにクラスを指定してインスタンスを取得すると、クラスと機能が分離できなくなります。

たとえば、Playerクラスに、移動とダメージ処理を実装していたとします。プレイヤーに関する処理なので、単一責任の原則に違反しているかは微妙なところです。ここで、それぞれの機能を利用するオブジェクトについて考えてみます。移動であれば、入力に応じた移動を指示するスクリプトからアクセスすることになるでしょう。この時、ダメージ処理の機能は必要ありません。一方、敵の弾などは、接触時にプレイヤーのダメージ処理が必要です。しかし、移動処理は必要ありません。

入力用の機能は、プレイヤーを操作するメソッドだけが必要で、敵の弾は、プレイヤーにダメージを与えるメソッドだけが必要です。このような場合に、移動用のメソッドをまとめたインターフェースと、ダメージを与えるためのメソッドをまとめたインターフェースを定義して、

利用ケースに応じて複数のインターフェースを定義することを
